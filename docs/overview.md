...
`seed4j-mcp` is a [Model Context Protocol](https://modelcontextprotocol.io) server that exposes [seed4j](https://github.com/seed4j) — an application generator — to MCP clients, AI coding assistants, autonomous agents, and custom host applications. Those MCP consumers call tools, read resources, or use prompts, and the server translates the work into HTTP requests against a **running seed4j instance**. seed4j is not embedded as a library.
...
- **Process model:** one long-lived Node.js process per MCP client connection. Launched by the MCP client or host application (Claude Desktop, an IDE plugin, a custom agent runner, …).
