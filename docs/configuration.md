...
| `SEED4J_CACHE_TTL_MS` | `3600000` (1 hour)      | TTL for the in-process catalogue cache covering `/api/modules`, `/api/modules-landscape`, and `/api/presets`. Must be a non-negative integer; set to `0` to disable caching (useful when iterating on seed4j itself). Errors are never cached. Call `refresh_catalogue` to clear the cache during a session.                  |
