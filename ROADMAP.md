...
Each feature lists: **What**, **Why**, **Where** (the files most likely touched), and **Done when** (acceptance criteria). When implementing one, also add/extend tests in `tests/` and update `README.md` / `CLAUDE.md` where the change is user- or agent-facing.
